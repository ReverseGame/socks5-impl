use socks5_impl::{
    ...
    server::{ auth, connection::associate, Server},
};
use std::{
...
use socks5_impl::server::connection::{ClientConnection, IncomingConnection};
use socks5_impl::server::connection::associate::{AssociatedUdpSocket, UdpAssociate};
...
pub struct CmdOpt {
    ...
    #[clap(short, long, value_name = "username", default_value="username")]
    username: Option<String>,

    /// Password for socks5 authentication.
    #[clap(short, long, value_name = "password", default_value="password")]
    ...
    match conn.wait_request().await? {
        ...
        ClientConnection::Connect(connect, addr) => {
            ...
            if let Ok(mut target) = target {
                ...
                log::trace!("{} -> {}", conn.stream.peer_addr()?, target.peer_addr()?);
                io::copy_bidirectional(&mut target, &mut conn.stream).await?;
            } else {
...
pub(crate) async fn handle_s5_upd_associate(associate: UdpAssociate<associate::NeedReply>) -> Result<()> {
    ...
    let listen_ip = associate.stream.local_addr()?.ip();
    ...
    match udp_listener.and_then(|socket| socket.local_addr().map(|addr| (socket, addr))) {
        ...
        Ok((listen_udp, listen_addr)) => {
            ...
            reply_listener.stream.shutdown().await?;
